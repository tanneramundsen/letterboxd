...
def calc_leadership_insularity(g, clusters):
    ...
    for i, s_g in enumerate(subgraphs):
        orig_g_indices = np.array([v['vertex_id'] for v in s_g.vs])
    ...
    cum_sum = 0
    pairwise_leadership_insularity = np.empty((len(clusters),len(clusters))) 
    for i in range(0,N_c,1):
        for j in range(i+1,N_c,1):
            if i not in disconnected_clusters and j not in disconnected_clusters:
                val = distance_between_leaders[i,j] \
                    / mean_distance_between_communities[i,j]
                pairwise_leadership_insularity[i,j] = val
                pairwise_leadership_insularity[j,i] = val
                cum_sum = cum_sum + (val * (len(clusters[i]) + len(clusters[j])))

    # Put it all together
    N_c_mod = N_c - len(disconnected_clusters)
    N_mod = N - len(vertices_in_disconnected_clusters)
    return ((1/(2 * N_c_mod * N_mod)) * cum_sum,pairwise_leadership_insularity)
